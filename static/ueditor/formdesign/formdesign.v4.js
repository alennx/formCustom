...
UE.plugins['text'] = function () {
    ...
    var formPlugins = el.getAttribute('plugins');
		if ( /input/ig.test( el.tagName ) && formPlugins==thePlugins) {
...
UE.plugins['macros'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        evt = evt || window.event;
        var el = evt.target || evt.srcElement;
        var formPlugins = el.getAttribute('formplugins');
        if ( /input/ig.test( el.tagName ) && formPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['radios'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
      var formPlugins = el.getAttribute('plugins');
        if ( /span/ig.test( el.tagName ) && formPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['select'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
      var formPlugins = el.getAttribute('plugins');
        if ( /select|span/ig.test( el.tagName ) && formPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['progressbar'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var formPlugins = el.getAttribute('formplugins');
        if ( /img/ig.test( el.tagName ) && formPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins[''] = function () {
    var me = this,thePlugins = '';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.FormDesignUrl+'/.html',
                name:thePlugins,
                editor:this,
                title: '表单设计器 - 清单',
...
UE.plugins['_template'] = function () {
    var me = this,thePlugins = '_template';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
...
UE.registerUI('button_',function(editor,uiName){
